...
<%
    String checking = (String) request.getAttribute("check");
 	...
 	else if(checking.equals("ok")){
 		out.println("<script>alert('검색결과를 찾았습니다.'); </script>");
 	}
	else if(checking.equals("empty")){
 		out.println("<script>alert('검색어를 입력하세요.'); </script>");
 		out.println("<script>location.href='/LibSystem/main'</script>");
 	}
%>
<body>
	...
	<p>환영합니다, <%out.print(member.getName());%>님</p>
		...
		<table width="1000" border="0" cellpadding="3" align="center"
			...
			<td width="200">
			<c:if test="${book.rent==null||book.rent.equals('')}">
				<form method='POST' action="rentdone">
				<input Type ="hidden" name ="id" value="<%out.print(member.getId());%>">
			...
			<c:if test="${book.rent!=null&&!book.rent.equals('')}">
				<c:if test="${book.rent.equals(member.getId())}">
				...
				<c:if test="${book.rent!=member.getId()}">
						대여불가
				</c:if>
			</c:if>
			</td>
